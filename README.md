...
On a `pull_request` (or `pull_request_target`) event, the composite action also posts a
comment on the PR itself, so a violation shows up where a reviewer is already looking
instead of only in a CI log nobody opens:
...
permissions:
  pull-requests: write   # required: the action posts/updates/deletes its own PR comment
  ...
  i18n-swarm:
    ...
    steps:
      ...
      - uses: greymoth-jp/i18n-swarm@v1
        # github-token defaults to the workflow's own GITHUB_TOKEN; only pass one
        # explicitly if the default token's permissions do not fit your setup.
```

The comment is sticky: the action looks for a comment it left on an earlier run (a hidden
`<!-- i18n-swarm:pr-comment -->` marker at the top, invisible when rendered) and updates
that one in place instead of posting a new comment on every push, so a PR with several
fixup commits still gets exactly one comment. If a later push fixes every flagged string,
the action deletes its own comment rather than leave a stale warning around. On a `push`
event, or any event without a PR to comment on, nothing changes: the step still fails on
exit code alone, same as before this existed.

The comment body is generated by a pure function (`buildPrComment` in `src/pr-comment.ts`,
also exported from the package) so its formatting has its own unit test, independent of
the GitHub API call that posts it. It lists the files and how many new strings landed in
each, one short before/after example pulled from the suggested fix, and a single quiet
attribution line at the bottom -- the whole point of the comment is that a reviewer who has
never heard of this tool sees it fail their PR and finds it from there, not an ad.
...
npx i18n-swarm check --comment out.md        also write the sticky PR-comment markdown to out.md
```
...
Once the workflow above is on the default branch, GitHub serves its status as an SVG for
free -- paste this at the top of your README (swap in your `owner/repo` and workflow
filename):

```md
[![i18n drift gate](https://github.com/<owner>/<repo>/actions/workflows/i18n-drift-gate.yml/badge.svg)](https://github.com/<owner>/<repo>/actions/workflows/i18n-drift-gate.yml)
```
